...
<div class="container-fluid px-4 py-4" style="max-width:900px;">
  <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
    ...
    <div class="card-header text-white border-0 py-3 px-4 d-flex justify-content-between align-items-center"
      ...
      <h5 class="mb-0 fw-bold"><i class="bi bi-shield-fill-check me-2"></i> Role List</h5>
      <div class="d-flex gap-2">
        <a href="<%=ORSView.ROLE_REPORT_CTL%>" target="_blank" class="btn btn-sm btn-warning fw-semibold">
          <i class="bi bi-file-earmark-pdf me-1"></i> Print PDF
        </a>
        <a href="<%=ORSView.ROLE_REPORT_CTL%>?type=doc" target="_blank" class="btn btn-sm btn-info fw-semibold">
          <i class="bi bi-file-earmark-word me-1"></i> Print DOC
        </a>
        <a href="<%=ORSView.ROLE_CTL%>" class="btn btn-sm btn-light text-primary fw-semibold">
    ...
    <form action="<%=ORSView.ROLE_LIST_CTL%>" method="POST">
      ...
      <div class="p-3 bg-light border-bottom d-flex flex-wrap gap-2 align-items-center">
        ...
        <button type="submit" name="operation" value="<%=BaseCtl.OP_DELETE%>" class="btn btn-danger btn-sm ms-auto"
                onclick="return confirm('Delete selected roles?')">
        ...
        <table class="table table-hover align-middle mb-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                <a href="RoleCtl?id=<%=bean.getId()%>" class="btn btn-sm btn-outline-primary">

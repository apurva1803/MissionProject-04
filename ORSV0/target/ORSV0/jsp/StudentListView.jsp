...
<div class="container-fluid px-4 py-4" style="max-width:1100px;">
  <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
    ...
    <div class="card-header text-white border-0 py-3 px-4 d-flex justify-content-between align-items-center"
      ...
      <h5 class="mb-0 fw-bold"><i class="bi bi-mortarboard-fill me-2"></i> Student List</h5>
      <div class="d-flex gap-2">
        <a href="<%=ORSView.STUDENT_REPORT_CTL%>" target="_blank" class="btn btn-sm btn-warning fw-semibold">
          <i class="bi bi-file-earmark-pdf me-1"></i> Print PDF
        </a>
        <a href="<%=ORSView.STUDENT_REPORT_CTL%>?type=doc" target="_blank" class="btn btn-sm btn-info fw-semibold">
    ...
    <form action="<%=ORSView.STUDENT_LIST_CTL%>" method="POST">
      ...
      <div class="p-3 bg-light border-bottom d-flex flex-wrap gap-2 align-items-center">
        <input type="text" name="firstName" class="form-control form-control-sm" style="max-width:160px;"
               placeholder="First Name" value="<%=ServletUtility.getParameter("firstName", request)%>">
        <input type="text" name="lastName" class="form-control form-control-sm" style="max-width:160px;"
               placeholder="Last Name" value="<%=ServletUtility.getParameter("lastName", request)%>">
        <input type="text" name="email" class="form-control form-control-sm" style="max-width:200px;"
               placeholder="Email ID" value="<%=ServletUtility.getParameter("email", request)%>">
        ...
        <table class="table table-hover align-middle mb-0">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><%=bean.getEmail()%></td>
              <td>
                <a href="StudentCtl?id=<%=bean.getId()%>" class="btn btn-sm btn-outline-primary">
